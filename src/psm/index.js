...
export default async (app, req, res) => {
    ...
    data.date = new Date()
    app.repository.create("LeadsFromPSM", data)
    ...
    if (data.type === "fillform") {
        ...
        if(contato._embedded && contato._embedded.leads)
            for (const lead_id of contato._embedded.leads) {
                ...
                if(lead.pipeline_id !== 12588032) continue
                await app.kommo.LeadUpdate(lead_id.id, {custom_fields_values})
                return await app.kommo.moveLeadToStatus(lead_id.id, 97219620)
            }
        return await createLead(app, 12588032, 97219620, contato.id, data.name, { custom_fields_values })
    }
};
